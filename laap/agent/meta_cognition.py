...
class MetaCognitionEngine:
    ...
    def suggest_mode_switch(self, context: Dict[str, Any]) -> Optional[str]:
        ...
        # 3. 高新颖性+高好奇心 → 探索模式
        if novelty > 0.7 and self.state.curiosity_level > 0.6:
    ...
    def detect_bias_in_response(self, response: str, 
        ...
        # 2. 确认偏差检测 — 是否只提到支持性的信息
        contradictory_phrases = [
            "但是", "另一方", "可能不是", "although", "however", 
            "on the other hand", "limitation", "caveat",
        ]
        has_contradictory = any(p in response_lower for p in contradictory_phrases)
        if not has_contradictory and len(response) > 200:
