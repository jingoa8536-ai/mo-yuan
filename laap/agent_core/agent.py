...
class Agent:
    ...
    def chat(self, message: str) -> str:
        ...
        if self.mode == AgentMode.KERNEL:
            return self._kernel_chat(message)
        elif self.mode == AgentMode.HERMES:
    ...
    def _kernel_chat(self, message: str) -> str:
        """kernel 模式的 chat 实现 (保留原 agent_core/agent.py 逻辑)。"""
        self.context.add(Role.USER, message)

        # Try llm.generate() first (test-compatible), fallback to llm.chat()
        try:
        ...
        self.context.add(Role.ASSISTANT, response if response else "")
        self.state = AgentState.DONE
        if self.memory:
            self.memory.remember_interaction(message, response or "")
        return response or "(空响应)"
    ...
    def stream_chat(self, message: str):
        ...
        """
        if self.mode == AgentMode.KERNEL:
            yield from self._kernel_stream_chat(message)
        elif self.mode == AgentMode.HERMES:
    ...
    def _kernel_stream_chat(self, message: str):
    ...
    def cognitive_status(self) -> dict:
        """认知层状态 (hermes 模式专属)。"""
        if self.mode != AgentMode.HERMES or not self._brain:
