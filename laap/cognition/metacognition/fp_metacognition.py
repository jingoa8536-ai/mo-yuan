...
class FirstPrinciplesMetaCognition:
    ...
    def ground_judgment(self, judgment: GroundedJudgment,
        ...
        depth = 0
        current = judgment
        grounding_chain = [judgment.claim]
        ...
        # 更新判断
        judgment.meta_level = max(judgment.meta_level, depth)
        judgment.first_principles_used.extend(grounding_chain[-3:])
        judgment.confidence = min(1.0, judgment.confidence + depth * 0.1)
    ...
    def _is_contradiction(self, claim: str) -> bool:
        ...
        for a, b in contradict_pairs:
            if a in claim.lower() and b in claim.lower():
                return True
        return False
    ...
    def _detect_fp_biases(self, claim: str, status: EpistemicStatus,
        ...
        if len(assumptions) == 0 and status not in (EpistemicStatus.FIRST_PRINCIPLE,
                                                      EpistemicStatus.EMPIRICAL_OBSERVATION):
            biases.append(FPCognitiveBias.UNEXAMINED_ASSUMPTION.value)
