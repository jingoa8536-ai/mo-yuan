...
class Sandbox:
    ...
    def run_code(self, code: str, context: Optional[Dict] = None) -> dict:
        ...
        try:
            ...
            logger.info("__SANDBOX_OK__")
            logger.info(json.dumps({{"type": "result", "value": str(_result)[:500]}}))
        except Exception as e:
            logger.info("__SANDBOX_ERR__")
            logger.error(json.dumps({{"type": "error", "value": str(e)[:500]}}))
        ''')
        try:
