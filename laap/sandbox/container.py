...
class CognitiveSandbox:
    ...
    def export_to(self, path: str | Path) -> None:
        """导出当前 sandbox 状态到 .laapsnap 文件（委托给 migration 模块）。

        序列化 8 个认知子系统状态，含 SHA256 完整性校验。
        文件格式：header(json) + payload(pickle)。

        Args:
            path: 目标文件路径（推荐 .laapsnap 扩展名）
        """
        from laap.sandbox.migration import export_sandbox
        export_sandbox(self, path)
    ...
    def import_from(
        ...
        """从 .laapsnap 文件导入并重建 sandbox（委托给 migration 模块）。
        ...
        """
        from laap.sandbox.migration import import_sandbox
